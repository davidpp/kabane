...
The project root is your working directory. Read what it says about itself before writing anything: CLAUDE.md and AGENTS.md, the package manifest's scripts and the gate the instructions name, .claude/agents/, any testing constraints, and how it names branches, worktrees and commits. Then fill every placeholder from what you read. The written skill must contain no {{ and nothing about a project that is not this one.

Write only under .claude/skills/dispatch/ in the project root. Show the full content of each file before writing it. If .claude/skills/dispatch/ already exists, show what would change and write nothing without an explicit yes. Claude Code reads .claude/skills; for Codex or Gemini, the same text goes into a section of AGENTS.md or GEMINI.md, and only when the human asks for that — it is the one other file you may write.
