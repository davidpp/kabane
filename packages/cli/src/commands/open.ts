 ...
 * day, and `cabane open JCAB-12` is what that should cost.
 */
export const open: Command = {
	name: "open",
	summary: "Open a task's linked issue in its app or the browser",
	usage: "cabane open <task-id>",
	run: async (args, ctx) => {
		...
		// Several links open the first, as the board's `O` does. `cabane show` lists them all.
